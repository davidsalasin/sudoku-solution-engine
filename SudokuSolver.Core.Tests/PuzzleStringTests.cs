...
{
    private readonly ISudokuSolver sudokuSolver = new DLX();

    const char MinNumericCharacter = '0';

    private static IList<int> StringToIntegerList(string input)
    {
        var inputList = new List<int>();
        for (var i = 0; i < input.Length; i++)
        {
            if (input[i] < MinNumericCharacter) continue;
    ...
    public void Solve_1x1_EMPTY_ReturnTrue()
    {
        ...
        Sudoku sp = new Sudoku(StringToIntegerList(puzzleString));

        // Act:
        bool solved = sudokuSolver.Solve(sp);
    ...
    public void Solve_4x4_UnvalidChar_ThrewException()
    {
        ...
        try
        {
            ...
            var sp = new Sudoku(StringToIntegerList(puzzleString));
            Assert.Fail();
        }
        catch
        {
